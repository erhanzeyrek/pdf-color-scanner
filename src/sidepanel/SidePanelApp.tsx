...
const SidePanelApp: React.FC = () => {
  ...
  // ── Listen for messages from background/viewer ──────────────
  useEffect(() => {
    ...
    >
      ...
      {!pdfLoaded && (
        ...
        >
          ...
          <p style={{ fontSize: '12px', lineHeight: '1.5' }}>
            Open a PDF file in Chrome. The extension will automatically redirect it to the viewer.
          </p>
        </div>
      )}
          ...
          <div style={{ padding: '12px 14px' }}>
            ...
            {selectedColor ? (
              ...
              >
                ...
                <span>Click <strong style={{ color: '#555' }}>Pick Color</strong> in the viewer toolbar, then click on the PDF</span>
              </div>
            )}
          </div>
        </div>
      ...
      {pdfLoaded && (
        ...
        >
              ...
              {TOLERANCE_OPTIONS.map((opt) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    border: '1px solid',
                    borderColor: tolerance === opt.value ? '#0061D3' : '#d0d0d0',
                    background: tolerance === opt.value ? '#e8f0fe' : '#fff',
                    color: tolerance === opt.value ? '#0061D3' : '#555',
                    fontSize: '11px',
                    fontWeight: tolerance === opt.value ? 600 : 400,
                    cursor: 'pointer',
                    fontFamily: 'inherit',
                    transition: 'all 0.1s',
                  }}
          ...
          <button
            onClick={handleScan}
            disabled={!selectedColor || scanning}
            style={{
              width: '100%',
              display: 'flex',
              alignItems: 'center',
              ...
              border: 'none',
              background:
                !selectedColor || scanning ? '#e0e0e0' : '#0061D3',
              color: !selectedColor || scanning ? '#aaa' : '#fff',
              cursor: !selectedColor || scanning ? 'not-allowed' : 'pointer',
              fontSize: '13px',
              fontWeight: 500,
              fontFamily: 'inherit',
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          {hasScanned && matches.length === 0 && !scanning && (
            ...
            >
              ...
              <p style={{ fontSize: '13px', fontWeight: 500, color: '#888', marginBottom: '4px' }}>
                No matches found
              </p>
              <p style={{ fontSize: '12px', lineHeight: 1.5 }}>
                Try increasing the tolerance, or pick a different color.
              </p>
            </div>
          )}
              ...
              >
                ...
                {selectedColor && (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontFamily: 'monospace', fontSize: '11px' }}>{hex!.toUpperCase()}</span>
                  </span>
                )}
              </div>
...
// ── Sub-component: Page Group ─────────────────────────────────
interface PageGroupProps {
  page: number;
  matches: PDFTextMatch[];
  onNavigate: (match: PDFTextMatch) => void;
}

const PageGroup: React.FC<PageGroupProps> = ({ page, matches, onNavigate }) => {
    ...
    <div style={{ borderBottom: '1px solid #f0f0f0' }}>
      ...
      {expanded &&
        matches.map((match, idx) => (
          <MatchItem key={idx} match={match} onNavigate={onNavigate} />
        ))}
    </div>
  );
};

// ── Sub-component: Match Item ─────────────────────────────────
interface MatchItemProps {
  match: PDFTextMatch;
  onNavigate: (match: PDFTextMatch) => void;
}

const MatchItem: React.FC<MatchItemProps> = ({ match, onNavigate }) => {
  const hex = rgbToHex(match.color);

  return (
    <button
      onClick={() => onNavigate(match)}
      title={`Navigate to "${match.text}" on page ${match.page}`}
      style={{
        width: '100%',
        display: 'flex',
